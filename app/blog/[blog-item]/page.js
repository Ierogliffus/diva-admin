...
import { createItem, getItemData, updateItem } from "@/api/blog"
...
export default function BlogItem({params}) {
    ...
    useEffect(() => {
        getItemData(params["blog-item"]).then(data => {
            setData(data)
            if (data.banner) {
                setPhoto(`${process.env.NEXT_PUBLIC_IMAGE_URL}${data.banner.photo_path}`)
            }
        })
    }, [])
    ...
    const save = (data) => {
        updateItem(params["blog-item"], data).then(res => console.log(res))
    }
                ...
                <Container fluid>
                    ...
                    <Form className="admin-form" onSubmit={handleSubmit(save)}>
                        <Form.Group className="admin-form-group" controlId="blog-title">
                            ...
                            <Form.Control {...register("title", {required: true})} className="admin-form-control" type="text" defaultValue={data.title} placeholder="Add blog title" />
                        </Form.Group>
                        <Form.Group className="admin-form-group" controlId="blog-banner">
                            <p className="admin-form-label">Banner</p>
                            <Form.Control type="file" {...register("banner", {required: false})} onChange={attachPhoto} className="admin-form-control-image" accept="image/png, image/jpeg" />
                        ...
                        <Form.Group className="admin-form-group" controlId="blog-content">
                            ...
                            <InputMarkdown form={{defaultValue: data.content, ...register("content", {required: true})}}/>
                        </Form.Group>
                        <Form.Group className="admin-form-group" controlId="blog-video">
                            <Form.Label className="admin-form-label">Link video</Form.Label>
                            <Form.Control defaultValue={data.video} {...register(`video`, {required: false, pattern: /^http[s]?:\/\/[\S]+$/i})} className="admin-form-control" type="text" placeholder="https://www.youtube.com/V00000000" />
                        </Form.Group>
